...
// locale (url) <-> Lang (iç kod) eşlemesi
const LOCALE_TO_LANG: Record<string, Lang> = { tr: "TR", en: "EN", de: "DE", nl: "NL", fr: "FR", es: "ES", it: "IT" };
const LANG_TO_LOCALE: Record<Lang, string> = { TR: "tr", EN: "en", DE: "de", NL: "nl", FR: "fr", ES: "es", IT: "it" };

export default function LandingClient({ locale }: { locale: string }) {
  const router = useRouter();
  const lang: Lang = LOCALE_TO_LANG[locale] || "TR";
  // Dil değişimi artık route değiştirir (SEO için ayrı URL)
  const setLang = (l: Lang) => router.push(`/${LANG_TO_LOCALE[l]}`);
